...
public class BoundingBoxE6 implements Parcelable, Serializable, MapViewConstants {
     ...
     *         {0,0} would be the upper left corner. {1,1} would be the lower right corner. {1,0} would be the lower left corner. {0,1} would be the
     *         upper right corner.
     */
    public PointF getRelativePositionOfGeoPointInBoundingBoxWithLinearInterpolation(final int aLatitude, final int aLongitude, final PointF reuse) {
    ...
    public PointF getRelativePositionOfGeoPointInBoundingBoxWithExactGudermannInterpolation(final int aLatitudeE6, final int aLongitudeE6,
            final PointF reuse) {
        final PointF out = (reuse != null) ? reuse : new PointF();
    ...
    public BoundingBoxE6 increaseByScale(final float pBoundingboxPaddingRelativeScale) {
        ...
        return new BoundingBoxE6(pCenter.getLatitudeE6() + mLatSpanE6Padded_2, pCenter.getLongitudeE6() + mLonSpanE6Padded_2, pCenter.getLatitudeE6()
    ...
    public String toString() {
        return new StringBuffer().append("N:").append(this.mLatNorthE6).append("; E:").append(this.mLonEastE6).append("; S:")
                .append(this.mLatSouthE6).append("; W:").append(this.mLonWestE6).toString();
    }
    ...
    public GeoPoint bringToBoundingBox(final int aLatitudeE6, final int aLongitudeE6) {
        return new GeoPoint(Math.max(this.mLatSouthE6, Math.min(this.mLatNorthE6, aLatitudeE6)), Math.max(this.mLonWestE6,
                Math.min(this.mLonEastE6, aLongitudeE6)));
    }

    public static BoundingBoxE6 fromLocations(final List<Location> locations, double padding) {
        double minLat = 90;
        double minLon = 180;
        double maxLat = -90;
        double maxLon = -180;
    ...
    public static BoundingBoxE6 fromGeoPoints(final ArrayList<? extends GeoPoint> partialPolyLine) {
        ...
        return new BoundingBoxE6(maxLat, maxLon, maxLat, maxLon);
    }
