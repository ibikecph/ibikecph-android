 ...
 * 
 */
public class MyLocationOverlay extends Overlay implements IMyLocationOverlay, IOverlayMenuProvider, SensorEventListener, LocationListener,
		Snappable {
	...
	public MyLocationOverlay(final Context ctx, final MapView mapView) {
		this(ctx, mapView, new ResourceProxy(ctx));
	}
	...
	public MyLocationOverlay(final Context ctx, final MapView mapView, final ResourceProxy pResourceProxy) {
		...
		PERSON_ICON = mResourceProxy.getBitmap(ResourceProxy.person);
		DIRECTION_ARROW = mResourceProxy.getBitmap(ResourceProxy.direction_arrow);
	...
	/**
	 * Set the minimum interval for location updates. See {@link LocationManager.requestLocationUpdates(String, long,
	 * float, LocationListener)}. Note that you should call this before calling {@link enableMyLocation()}.
	 * 
	 * @param milliSeconds
	 */
	...
	/**
	 * Set the minimum distance for location updates. See {@link LocationManager.requestLocationUpdates}. Note that you
	...
	protected void drawMyLocation(final Canvas canvas, final MapView mapView, final Location lastFix) {
		...
		if (mDrawAccuracyEnabled) {
			final float radius = lastFix.getAccuracy()
					/ (float) TileSystem.GroundResolution(lastFix.getLatitude(), mapView.getZoomLevelFloor());
	...
	protected Rect getMyLocationDrawingBounds(int zoomLevel, Location lastFix, Rect reuse) {
		...
		if (lastFix.hasBearing()) {
			...
			int widestEdge = (int) Math.ceil(Math.max(DIRECTION_ARROW.getWidth(), DIRECTION_ARROW.getHeight()) * Math.sqrt(2));
		...
		if (mDrawAccuracyEnabled) {
			...
			final int strokeWidth = (int) Math.ceil(mCirclePaint.getStrokeWidth() == 0 ? 1 : mCirclePaint.getStrokeWidth());
	...
	public boolean onCreateOptionsMenu(final Menu pMenu, final int pMenuIdOffset, final MapView pMapView) {
		pMenu.add(0, MENU_MY_LOCATION + pMenuIdOffset, Menu.NONE, mResourceProxy.getString(R.string.my_location)).setIcon(
				mResourceProxy.getDrawable(ResourceProxy.ic_menu_mylocation));

		pMenu.add(0, MENU_COMPASS + pMenuIdOffset, Menu.NONE, mResourceProxy.getString(R.string.compass)).setIcon(
				mResourceProxy.getDrawable(ResourceProxy.ic_menu_compass));
	...
	/**
	 * Enables "follow" functionality. The map will center on your current location and automatically scroll as you
	...
	public void enableFollowLocation() {
		...
		if (isMyLocationEnabled()) {
			...
			if (mLocation != null) {
				TileSystem.LatLongToPixelXY(mLocation.getLatitude(), mLocation.getLongitude(), MapViewConstants.MAXIMUM_ZOOMLEVEL,
						mMapCoords);
				final int worldSize_2 = TileSystem.MapSize(MapViewConstants.MAXIMUM_ZOOMLEVEL) / 2;
				mMapCoords.offset(-worldSize_2, -worldSize_2);
	...
	/**
	 * If enabled, the map will center on your current location and automatically scroll as you move. Scrolling the map
	 * in the UI will disable.
	 * 
	 * @return true if enabled, false otherwise
	 */
	...
	/**
	 * Enable location updates and show your current location on the map. By default this will request location updates
	 * as frequently as possible, but you can change the frequency and/or distance by calling {@link
	 * setLocationUpdateMinTime(long)} and/or {@link setLocationUpdateMinDistance(float)} before calling this method.
	 * You will want to call enableMyLocation() probably from your Activity's Activity.onResume() method, to enable the
	 * features of this overlay. Remember to call the corresponding disableMyLocation() in your Activity's
	...
	/**
	 * Enable orientation sensor (compass) updates and show a compass on the map. You will want to call enableCompass()
	 * probably from your Activity's Activity.onResume() method, to enable the features of this overlay. Remember to
	 * call the corresponding disableCompass() in your Activity's Activity.onPause() method to turn off updates when in
	 * the background.
	 */
	@Override
	public boolean enableCompass() {
		...
		if (mSensorListener == null) {
			...
			result = mSensorListener.startListening(this, Sensor.TYPE_ORIENTATION, SensorManager.SENSOR_DELAY_UI);
		}
