...
public class CompassOverlay extends SafeDrawOverlay implements IOverlayMenuProvider, IOrientationConsumer {
	...
	public CompassOverlay(final Context context, IOrientationProvider orientationProvider, final MapView mapView) {
		this(context, orientationProvider, mapView, new ResourceProxy(context));
	}
	...
	public boolean onCreateOptionsMenu(final Menu pMenu, final int pMenuIdOffset, final MapView pMapView) {
		pMenu.add(0, MENU_COMPASS + pMenuIdOffset, Menu.NONE, mResourceProxy.getString(R.string.compass))
				.setIcon(mResourceProxy.getDrawable(ResourceProxy.ic_menu_compass)).setCheckable(true);
	...
	/**
	 * Enable receiving orientation updates from the provided IOrientationProvider and show a compass on the map. You
	 * will likely want to call enableCompass() from your Activity's Activity.onResume() method, to enable the features
	 * of this overlay. Remember to call the corresponding disableCompass() in your Activity's Activity.onPause() method
	 * to turn off updates when in the background.
	 */
