...
public class ScaleBarOverlay extends SafeDrawOverlay implements GeoConstants {
	...
	public ScaleBarOverlay(final Context context) {
		this(context, new ResourceProxy(context));
	}
	...
	/**
	 * Sets the scale bar screen offset for the bar. Note: if the bar is set to be drawn centered, this will be the
	 * middle of the bar, otherwise the top left corner.
	 * 
	 * @param x
	 *            x screen offset
	 * @param y
	...
	/**
	 * Flag to draw the bar centered around the set offset coordinates or to the right/bottom of the coordinates
	 * (default)
	 * 
	 * @param centred
	...
	/**
	 * If enabled, the bar will automatically adjust the length to reflect a round number (starting with 1, 2 or 5). If
	 * disabled, the bar will always be drawn in full length representing a fractional distance.
	 */
	...
	/**
	 * Sets the maximum bar length. If adjustLength is disabled this will match exactly the length of the bar. If
	 * adjustLength is enabled, the bar will be shortened to reflect a round number in length.
	 * 
	...
	public void drawSafe(final ISafeCanvas c, final MapView mapView, final boolean shadow) {
		...
		// TODO - we should be using floating point zoom levels...
		final int zoomLevel = mapView.getZoomLevelFloor();
	...
	private void createScaleBarPicture(final MapView mapView) {
		...
		// paint background
		if (bgPaint != null) {
			canvas.drawRect(0, 0, yTextRect.height() + 2 * lineWidth + yTextSpacing, xTextRect.height() + 2 * lineWidth + xTextSpacing,
					bgPaint);
			if (latitudeBar)
				canvas.drawRect(yTextRect.height() + 2 * lineWidth + yTextSpacing, 0, xBarLengthPixels + lineWidth, xTextRect.height() + 2
						* lineWidth + xTextSpacing, bgPaint);
			if (longitudeBar)
				canvas.drawRect(0, xTextRect.height() + 2 * lineWidth + xTextSpacing, yTextRect.height() + 2 * lineWidth + yTextSpacing,
						yBarLengthPixels + lineWidth, bgPaint);
		}

		// draw latitude bar
		if (latitudeBar) {
			canvas.drawRect(0, 0, xBarLengthPixels, lineWidth, barPaint);
			canvas.drawRect(xBarLengthPixels, 0, xBarLengthPixels + lineWidth, xTextRect.height() + lineWidth + xTextSpacing, barPaint);

			if (!longitudeBar) {
				canvas.drawRect(0, 0, lineWidth, xTextRect.height() + lineWidth + xTextSpacing, barPaint);
			}

			canvas.drawText(xMsg, xBarLengthPixels / 2 - xTextRect.width() / 2, xTextRect.height() + lineWidth + xTextSpacing, textPaint);
		}

		// draw longitude bar
		if (longitudeBar) {
			canvas.drawRect(0, 0, lineWidth, yBarLengthPixels, barPaint);
			canvas.drawRect(0, yBarLengthPixels, yTextRect.height() + lineWidth + yTextSpacing, yBarLengthPixels + lineWidth, barPaint);

			if (!latitudeBar) {
				canvas.drawRect(0, 0, yTextRect.height() + lineWidth + yTextSpacing, lineWidth, barPaint);
			}
	...
	/**
	 * Returns a reduced length that starts with 1, 2 or 5 and trailing zeros. If set to nautical or imperial the input
	 * will be transformed before and after the reduction so that the result holds in that respective unit.
	 * 
	 * @param length
	 *            length to round
	...
	protected String scaleBarLengthText(final int meters, final boolean imperial, final boolean nautical) {
		if (this.imperial) {
			if (meters >= METERS_PER_STATUTE_MILE * 5) {
				return resourceProxy.getString(R.string.format_distance_miles, (int) (meters / METERS_PER_STATUTE_MILE));

			} else if (meters >= METERS_PER_STATUTE_MILE / 5) {
				return resourceProxy.getString(R.string.format_distance_miles, ((int) (meters / (METERS_PER_STATUTE_MILE / 10.0))) / 10.0);
			} else {
				return resourceProxy.getString(R.string.format_distance_feet, (int) (meters * FEET_PER_METER));
			}
		} else if (this.nautical) {
			if (meters >= METERS_PER_NAUTICAL_MILE * 5) {
				return resourceProxy.getString(R.string.format_distance_nautical_miles, ((int) (meters / METERS_PER_NAUTICAL_MILE)));
			} else if (meters >= METERS_PER_NAUTICAL_MILE / 5) {
				return resourceProxy.getString(R.string.format_distance_nautical_miles,
			...
			if (meters >= 1000 * 5) {
				return resourceProxy.getString(R.string.format_distance_kilometers, (meters / 1000));
			} else if (meters >= 1000 / 5) {
				return resourceProxy.getString(R.string.format_distance_kilometers, (int) (meters / 100.0) / 10.0);
			} else {
				return resourceProxy.getString(R.string.format_distance_meters, meters);
			}
		}
	}
